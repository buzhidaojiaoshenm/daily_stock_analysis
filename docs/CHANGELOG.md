...
- [改进] 定时任务新增 `SCHEDULE_TIMEZONE` 显式时区配置，避免 Docker 或跨时区部署时依赖宿主机本地时区。
...
- [改进] 搜索层新闻结果新增轻量舆情标注，输出风险标签、催化标签与情绪评分，供 Agent 情报工具直接消费。
- [改进] Agent 多轮对话历史压缩会保留被省略旧消息的本地摘要，减少长会话上下文丢失。
- [改进] API 契约新增 OpenAPI/前端类型生成与漂移检查，异步分析任务补齐取消、超时、容量限制、重试计数和失败分类状态。
