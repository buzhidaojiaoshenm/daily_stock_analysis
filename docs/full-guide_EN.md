...
| `ANALYSIS_TASK_QUEUE_LIMIT` | Async analysis task capacity, counted as `pending` + `processing`; `0` disables the guard | `100` |
| `ANALYSIS_TASK_TIMEOUT_SECONDS` | Seconds before an async analysis task is marked `timeout`; `0` disables automatic timeout marking | `0` |
| `ANALYSIS_TASK_MAX_RETRIES` | Additional retry attempts after async analysis task failures; retry metadata is exposed through `retry_count` | `0` |
...
> Task-state persistence note: async analysis tasks are now written to database state snapshots. `/api/v1/analysis/tasks` and `/api/v1/analysis/status/{task_id}` use the current in-memory queue first, then fall back to recently persisted task state when the task is no longer in memory. After a service-process restart, `pending` / `processing` tasks left by the previous process stay visible and are marked `failed` with a retry message; completed reports still use analysis history as the source of truth. Task statuses include `pending`, `processing`, `completed`, `failed`, `cancelled`, and `timeout`; terminal failures expose `failure_type` when available (`validation` / `data_source` / `llm` / `notification` / `timeout` / `internal` / `cancelled`) plus `retry_count` / `max_retries`.

> Progress-stream note: `GET /api/v1/analysis/tasks/stream` now emits `task_progress`, `task_retrying`, `task_cancelled`, and `task_timeout` in addition to `task_created / task_started / task_completed / task_failed`. The regular analysis path updates `progress` and `message` across quote preparation, news retrieval, context assembly, LLM generation, and report persistence. Streaming chunks are accumulated only on the server side; history is persisted only after the final JSON parses successfully. If streaming is unavailable before the first chunk, the system falls back to the previous non-stream request. If a stream fails after partial output has already arrived, the system first retries non-stream for the same model, then continues through existing fallback models in the original order (primary + fallback list).
...
# Trigger market review (region: cn / us / both; notification is disabled by default)
