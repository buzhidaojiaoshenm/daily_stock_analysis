...
> Task-state persistence note: async analysis tasks are now written to database state snapshots. `/api/v1/analysis/tasks` and `/api/v1/analysis/status/{task_id}` use the current in-memory queue first, then fall back to recently persisted task state when the task is no longer in memory. After a service-process restart, `pending` / `processing` tasks left by the previous process stay visible and are marked `failed` with a retry message; completed reports still use analysis history as the source of truth.
