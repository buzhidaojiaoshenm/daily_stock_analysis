...
3. **应用交易技能** — 评估每个激活技能的条件，并在 `dashboard.strategy_basis` 中体现启用、命中、未命中策略与判断摘要。
...
{{
    ...
    "dashboard": {{
        ...
        "strategy_basis": {{
            "active_skills": ["本次启用的策略或技能名称"],
            "matched_skills": ["主要命中的策略或技能名称"],
            "rejected_skills": ["评估后未命中或暂不适用的策略或技能名称"],
            "rationale": "用1-2句话说明策略命中/未命中的关键依据"
        }},
        "data_perspective": {{
...
6. **策略依据可追踪**：必须说明启用了哪些策略、哪些条件命中、哪些暂未命中
